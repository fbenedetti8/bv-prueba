...
class Checkout extends MY_Controller {
	...
	public function orden($hash){
		...
		if(is_array($data) && count($data) == 2){
			...
			if($orden->id){
				...
				//si la orden tiene elegida un tipo de moneda por parte del usuario, la uso para mostrar el tipo de moneda en front
				/*
				if($orden->user_tipo_moneda){
					$precio_usd = $orden->user_tipo_moneda;
				}
				else{
					//sino está seteada, entonces primero veo si hay alguna seleccion en sesion 
					$precio_usd = $this->session->userdata('user_tipo_moneda') ? $this->session->userdata('user_tipo_moneda') : ($this->data['combinacion']->precio_usd?'USD':'ARS');

					$this->Orden->update($orden->id,array('user_tipo_moneda'=>$precio_usd));
					$orden->user_tipo_moneda = $precio_usd;
				}
				*/
				//28-02-19 Juan me confirmo que el precio se muestra en la moneda del viaje como hasta ahora
				$precio_usd = $this->data['combinacion']->precio_usd?'USD':'ARS';

				$this->data['orden'] = $orden;

				$this->data['precios'] = calcular_precios_totales($this->data['combinacion'],$this->data['adicionales_valores'],$precio_usd,$reserva_id=false,$orden->id);
				...
				if(@$this->data['destino']->imagen_mobile && file_exists('./uploads/destinos/'.$this->data['destino']->id.'/'.$this->data['destino']->imagen_mobile))
					$this->seo_image = base_url().'uploads/destinos/'.$this->data['destino']->id.'/'.$this->data['destino']->imagen_mobile;
	...
	// Maxi | 11-01-19 El paso 2 es ahora el 3 y el 3 es el 2
	// Paso 2: pide datos de acompañantes
	// Paso 3: pide datos de facturacion
	function cargar_paso($nro){
		...
		$adicionales = $this->adicionales_orden($this->data['orden']);
		$this->data['adicionales'] = $adicionales;
		...
		switch($nro){
			case 1:
				...
				//17-08-18 si se alcanzó la fecha limite de carga de datos basicos, los pido
				...
				//obtengo los adicionales del pasajero responsable, si es que no fueron asignados a todos
				$this->load->model('Orden_pasajero_adicional_model','Pax_adicionales');
				$adic = $this->Pax_adicionales->getWhere(['orden_pasajero_id' => $this->data['responsable']->id])->result();
				$this->data['responsable_adicionales'] = [];
				foreach ($adic as $a) {
					$this->data['responsable_adicionales'][] = $a->id;
				}
			...
			case 2:
				...
				foreach($this->data['acompanantes'] as $a){
					...
					$adic = $this->Pax_adicionales->getWhere(['orden_pasajero_id' => $a->id])->result();
				...
				if(!validateDate($str_fecha)){
					...
					if(!isValidEmail($_POST['email'])){
						...
						if(isset($nacionalidad_id) && $nacionalidad_id == 1 ) {
							...
							//checqueo si el pax responsable puedo asignarle el adicional

							if( isset($pax_adicional) && count($pax_adicional) ){
								$this->load->model('Orden_pasajero_adicional_model','Pax_adicionales');

								foreach ($pax_adicional as $ad_id => $state) {
									//me fijo que cantidad de ese adicional estan siendo usados por los pax de la orden
									$q = "select count(*) as cantidad, ifnull(oa.cantidad,0) as totales, case when x.id then 1 else 0 end as usuario 
											from bv_ordenes_pasajeros_adicionales pa
											join bv_ordenes_pasajeros op on op.id = pa.orden_pasajero_id 
											join bv_ordenes o on o.id = op.orden_id and o.id = ".$orden_id."
											join bv_ordenes_adicionales oa on oa.orden_id = o.id
											left join bv_ordenes_pasajeros_adicionales x on x.orden_pasajero_id = pa.orden_pasajero_id and x.orden_pasajero_id  = ".$pax_id." and x.adicional_id = ".$ad_id." 
											where pa.adicional_id = ".$ad_id;
									$res = $this->db->query($q)->row();
								
									$puede = (!$res->cantidad && !$res->totales) || ($res->cantidad < $res->totales) || $res->usuario;

									if(!$puede){
										$ret['status'] = 'error';
										$ret['pax_invalid_adicionales'] = true;
										$ret['msg'] = 'Ya seleccionaste la cantidad total de adicionales.';
										$ret['fields'] = [];
										$ret['field'] = 'pasajero_adicional_'.$pax_id;
										echo json_encode($ret);
										exit();
									}

									$this->Pax_adicionales->deleteWhere(['orden_pasajero_id' => $pax_id]);
									if($state){
										$arr = array('orden_pasajero_id' => $pax_id, 'adicional_id' => $ad_id);
										$this->Pax_adicionales->insert($arr);
									}
							...
							*/
							//06-03-19 el dino ahora puede ser completado por extranjero tambien, pero no es obligatorio
							$pax['dni'] = isset($dni) ? $dni : '';
							...
							//06-03-19 si el pax pudo completar los datos xq por ejemplo es extranjero y el paq alcanzo la fecha de datos completo..						
							...
							*/
							...
							//actualizo los adicionales de cada pasajero 
							if( isset($pax_adicional) && count($pax_adicional) ){
								$this->load->model('Orden_pasajero_adicional_model','Pax_adicionales');
							...
							// Maxi | 11-01-19 si la orden es de 1 solo pax, redirijo directamente al paso 3
	...
	function validar_paso3(){
		...
		//si saltea el paso 2 y completa luego, no valido datos
		if(isset($completa_luego) && $completa_luego){
			...
			//update de marca de que salteó el paso 2
			$this->Orden->update($orden_id,array('paso_actual'=>3, 'salteo_paso2'=>1));
			$orden->paso_actual = 3;
			$orden->salteo_paso2 = 1;
			...
			/*
			// Maxi - 11-01-19 esto ya no es parte de la validacion del paso 2
			...
			*/
			...
			if (($this->data['combinacion']->agotada && $this->data['combinacion']->habitacion_id!=99) || !$paquete->cupo_disponible || ($paquete->cupo_paquete_personalizado && $paquete->cupo_paquete_disponible < $this->data['orden']->pasajeros) || @$this->data['habitacion_sin_cupo'] || @$this->data['transporte_sin_cupo']) {
				$this->efectivizar_orden($orden_id);
			}
			*/
			...
			if(count($incompletos)){
				if($orden->pasajeros == $pasajero->numero_pax){
					$this->Orden->update($orden_id,array('paso_actual'=>3, 'completo_paso2'=>0, 'salteo_paso2'=>1));
				...
				//si TODOS los pasajeros (o al menos 1) estan con marca de salteados (no quiso completar) => paso 3 con salteo de paso2
				if(count($incompletos_salteados) <= ($this->data['orden']->pasajeros-1)){
					$this->Orden->update($orden_id,array('paso_actual'=>3, 'completo_paso2'=>0, 'salteo_paso2'=>1));
				}
				else{
					//update de marca de que completo paso 2
					$this->Orden->update($orden_id,array('paso_actual'=>3, 'completo_paso2'=>1, 'salteo_paso2'=>0));
				}
		...
		// ini_set('display_errors', 1);
			...
			if($pax_id > 0){
				...
				//esto es por el tema de grupales que se subdividen
				//chequeo si existe reserva de ese pasajero EMAIL para esta combinacion
				$wh = array();
				$wh['p.email'] = $_POST['email_'.$pax_id];
				$wh['r.paquete_id'] = $orden->paquete_id;
				$existe = $this->Reserva_pasajero->getWhere($wh)->result();
				...
				//17-08-18
				//si alcanzó la fecha limite de carga de datos, TODOS los datos de pax son obligatorios
				...
				//12-03-19 valido si el pax quiere agregar algun adicional y ya no puede porque los eligio para otros pax
				
				//actualizo los adicionales de cada pasajero 
				$pax_adicional = isset($_POST['pax_adicional_'.$pax_id]) ? $_POST['pax_adicional_'.$pax_id] : [];
				if( isset($pax_adicional) && count($pax_adicional) ){
					$this->load->model('Orden_pasajero_adicional_model','Pax_adicionales');
				...
				*/
				...
				if(!$pax['emergencia_nombre']){
					unset($pax['emergencia_nombre']);
				}
				
				if(!$pax['emergencia_telefono_codigo']){
					unset($pax['emergencia_telefono_codigo']);
				}
				
				if(!$pax['emergencia_telefono_numero']){
					unset($pax['emergencia_telefono_numero']);
				}
				...
				*/
				...
				//08-04-19 si no alcanzo la fecha limite y no completo los datos del pasaporte los saco de la validacion
				/*if( $orden->fecha_limite_completar_datos > date('Y-m-d')){
					if(!$pax['pasaporte']){
						unset($pax['pasaporte']);
					}
					if(!$pax['pais_emision_id']){
						unset($pax['pais_emision_id']);
					}
					if(!$pax['fecha_emision']){
						unset($pax['fecha_emision']);
					}
					if(!$pax['fecha_vencimiento']){
						unset($pax['fecha_vencimiento']);
					}					
				}*/

				$pax_validar = $pax;
				
				//06-03-19 si no es obligatorio el pasaporte, entonces no los considero para validar como completos los datos del pax
				if(!$pasaporte_obligatorio){
					unset($pax_validar['pasaporte']);
					unset($pax_validar['pais_emision_id']);
					unset($pax_validar['fecha_emision']);
					unset($pax_validar['fecha_vencimiento']);
				}

				if( !$validar_dni ){
					unset($pax_validar['dni']);
				}

				$pax['completo'] = 1;	

				foreach ($pax_validar as $key=>$value) {
					if (empty($pax_validar[$key])) {
						$pax['completo'] = 0;
						break;
					}
				}
				...
				if(count($incompletos)){
					...
					//si acabo de guardar datos del ultimo pasajero y hay incompletos todavia, simulo que salteo paso 2
					if($orden->pasajeros == $pasajero->numero_pax){
						$this->Orden->update($orden_id,array('paso_actual'=>3, 'completo_paso2'=>0, 'salteo_paso2'=>1));
				...
				if ($this->form_validation->run() == FALSE){
					...
					$this->Orden->update($orden_id,array('fecha_completo_paso2' => $fecha_completo_paso2, 'completo_paso2' => 1, 'paso_actual' => 3));
					$this->data['orden'] = $this->Orden->get($orden_id)->row();
	...
	function validar_paso2(){
		...
		if(isset($saltear_facturacion) && $saltear_facturacion){
			...
			if($this->data['orden']->pasajeros == 1){
				...
				$precio_usd = $this->data['combinacion']->precio_usd?'USD':'ARS';
				/*
				if($this->data['orden']->user_tipo_moneda){
					$precio_usd = $this->data['orden']->user_tipo_moneda;
				}
				*/

				$this->data['precios'] = calcular_precios_totales($this->data['combinacion'],$this->data['adicionales_valores'],$precio_usd);
			...
			*/
			...
			if ($this->form_validation->run() == FALSE){
				...
				if(!isValidCuit($cuitcompleto)){
					...
					/*
					// Maxi | 11-01-19 esto ya no es parte del paso 3 (antes 2)
					//si la orden es por 1 solo pasajero (no hay acompañantes) voy directo al paso de pagar
					if($this->data['orden']->pasajeros == 1){
						$fecha_completo_paso3 = date('Y-m-d H:i:s');
						$this->Orden->update($orden_id,array('completo_paso2' => 1, 'completo_paso3' => 1, 'fecha_completo_paso3' => $fecha_completo_paso3, 'paso_actual' => 4));
					...
					*/
					...
					$fecha_completo_paso3 = date('Y-m-d H:i:s');
					$this->Orden->update($orden_id,array('completo_paso3' => 1, 'fecha_completo_paso3' => $fecha_completo_paso3, 'paso_actual' => 4));
					...
					//si la orden es por 1 solo pasajero (no hay acompañantes) voy directo al paso de pagar
					// Maxi | 11-01-19 ahora desde esta validacion va al paso 4 siempre
					//if($this->data['orden']->pasajeros == 1){
						$this->data['combinacion'] = $this->Combinacion->get($this->data['orden']->combinacion_id)->row();
						$this->data['orden']->adicionales = $this->adicionales_orden($this->data['orden']);
						$this->data['hash'] = encriptar($this->data['orden']->code);
	...
	function validar_paso4(){
		...
		//pago total
		if( $pago == "total" ){
			$monto = $precios['precio_total'];//el precio a pagar
			$ret['monto'] = $monto;
			$ret['status'] = "success";
			...
			if($monto == "" || $monto <= 0){
				...
				$ret['msg'] = "El monto mínimo de pago es de ".$minimo;
				$ret['status'] = "error";
				echo json_encode($ret);
			}
			else{
