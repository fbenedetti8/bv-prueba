...
class WSAA {
  ...
  const CERT = "keys/ghf_test.crt";        # The X.509 certificate in PEM format
   ...
   */
  private $path = './application/libraries/afip/';
   ...
   * Constructor
   */
  public function __construct($path = '../application/libraries/afip/', $service = 'wsfe') 
  {
  ...
  private function create_TRA()
  {
    ...
    $TRA->header->addChild('generationTime', date('c',date('U')-240));//120
  ...
  {     
    ...
    // para logueo
    file_put_contents($this->path."xmlgenerados/request-loginCms.xml", $this->client->__getLastRequest());
    file_put_contents($this->path."xmlgenerados/response-loginCms.xml", $this->client->__getLastResponse());
  ...
  public function get_expiration() 
  {
    ...
    if(empty($this->TA)) {
      if (!file_exists($this->path.self::TA)) return FALSE;
